...
<table class="compare-list" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) )
	...
	<?php if ( empty( $products ) ) : ?>
		...
		<tr class="no-products">
			<td><?php esc_html_e( 'No products added in the compare table.', 'learts' ) ?></td>
		</tr>
			...
			<?php foreach ( $products as $product_id => $product ) :
				...
				<td class="<?php echo esc_attr( $product_class ); ?>">
					<a href="<?php echo add_query_arg( 'redirect', 'view', $this->remove_product_url( $product_id ) ) ?>"
					   data-product_id="<?php echo esc_attr( $product_id ); ?>"><?php _e( 'Remove', 'learts' ) ?>
				</td>
			<?php endforeach ?>
		</tr>
				...
				<?php foreach ( $products as $product_id => $product ) :
					...
					<td class="<?php echo esc_attr( $product_class ); ?>"><?php
						switch ( $field ) {
							...
							case 'image':
								echo '<div class="image-wrap">' . wp_get_attachment_image( $product->fields[ $field ], apply_filters( 'learts_compare_image_size', 'yith-woocompare-image' ) ) . '</div>';
								break;

							case 'price':
								$price_html = $product->get_price_html();

								echo  wp_kses( $price_html, Learts_Helper::wp_kses_allowed_html('learts-price') );
								break;

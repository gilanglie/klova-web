...
class VI_WOO_ORDERS_TRACKING_DATA {
	...
	public function __construct() {
		...
		$this->default = array(
			...
			'shipping_carriers_define_list' => json_encode( $this->shipping_carriers() ),
	...
	public function get_shipping_carrier_by_slug( $slug, $type = '', $check_service = true ) {
		...
		switch ( $type ) {
			case 'custom-carrier':
				...
				break;
			case 'define-carrier':
				$carriers = $this->shipping_carriers();
				foreach ( $carriers as $carrier ) {
					if ( $carrier['slug'] === $slug ) {
						$carrier['carrier_type'] = $type;
						$result                  = $carrier;
	...
	public function shipping_carriers() {
		$carriers = array(
			...
			array(
				'name'               => 'Correos Chile',
				'slug'               => 'correos-chile',
				'url'                => 'https://seguimientoenvio.correos.cl/home/index/{tracking_number}',
				'country'            => 'CL',
				'active'             => '',

...
if ( ! class_exists( 'YITH_Pre_Order_Frontend_Premium' ) ) {
	...
	class YITH_Pre_Order_Frontend_Premium extends YITH_Pre_Order_Frontend {
		...
		public function print_availability_date( $class, $timestamp, $style, $availability_label_product = '' ) {
			...
			if ( ! empty( $timestamp ) ) {
				...
				if ( empty( $availability_label ) ) {
					$availability_label = apply_filters( 'yith_ywpo_default_availability_date_label',
						sprintf( __( 'Available on: %s at %s', 'yith-woocommerce-pre-order' ),
		...
		public function show_date_on_loop() {
			...
			echo $this->print_availability_date( 'pre_order_loop', $timestamp, $style, $pre_order->get_pre_order_availability_date_label() );
		...
		public function availability_date( $product_id ) {
			...
			return $this->print_availability_date( 'pre_order_single', $timestamp, $style, $pre_order->get_pre_order_availability_date_label() );
		...
		public function show_date_on_cart( $cart_item ) {
			...
			if ( $is_pre_order == 'yes' && apply_filters( 'yith_wcpo_allow_other_page', is_cart() ) ) {
				...
				echo $this->print_availability_date( 'pre_order_on_cart', $timestamp, $style, $pre_order->get_pre_order_availability_date_label() );
		...
		public function variable_product_label() {
			...
			if ( $all_children_are_pre_order ) {
				...
				apply_filters( 'yith_ywpo_variable_product_label', $message );
				echo $message;
			}
		}

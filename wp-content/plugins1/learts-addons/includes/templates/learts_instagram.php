 ...
 *
 * Shortcode class
 * @var $this WPBakeryShortCode_Learts_Instagram
 */
...
$class = Learts_VC::get_grid_item_class( array(
	'xs' => 2,
	'sm' => 2,
...
<div class="<?php echo esc_attr( trim( $css_class ) ); ?>"
	...
	<?php if ( ! empty( $username ) ) {
		...
		$media_array    = Learts_Instagram::scrape_instagram( $username, $number_items, $square_media == 'yes' );
		...
		<?php } else { ?>
			<div class="tm-instagram-pics row" style="margin: 0 -<?php echo esc_attr( $spacing / 2 ); ?>px">
				<?php foreach ( $media_array as $item ) { ?>
					<div class="item<?php echo ' ' . $class; ?>"
						...
						<img src="<?php echo esc_url( $item['thumbnail'] ); ?>" alt="image" class="item-image"/>
						<?php if ( 'video' == $item['type'] ) { ?>
							<span class="play-button"><?php esc_html_e( 'Play', 'learts-addons' ) ?></span>
						<?php } ?>
